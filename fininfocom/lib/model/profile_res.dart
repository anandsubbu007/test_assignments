...
class Results {
  ...
  int get since {
    final regOn =
        DateTime.tryParse(registered?.date.toString() ?? '') ?? DateTime.now();
    return DateTime.now().difference(regOn).inDays.abs();
  }

  String get dateOfBirth {
    final on = DateTime.tryParse(registered?.date.toString() ?? '');
    if (on == null) return '';
    return "${on.day}/${on.month}/${on.year}";
  }
